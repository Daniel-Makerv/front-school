...
<div class="container">
    <!-- Table -->
    <h class="text-2xl font-semibold">Profesores</h1>
    <div class="w-full mx-auto bg-white shadow-lg rounded-sm border border-gray-200">
        <header class="px-5 py-4 border-b border-gray-100">
            <h2 class="font-semibold text-gray-800">Registros de Profesores</h2>
        </header>
        <div class="p-3">
                ...
                <table class="table-auto w-full">
                    <thead class="text-xs font-semibold uppercase text-gray-400 bg-gray-50">
                        <tr>
                            ...
                            <th class="p-2 whitespace-nowrap">
                                <div class="font-semibold text-left">Nombre profesor</div>
                            </th>
                            <th class="p-2 whitespace-nowrap">
                                <div class="font-semibold text-left">Salon de clases</div>
                            </th>
                            <th class="p-2 whitespace-nowrap">
                                <div class="font-semibold text-left">Ciclo Escolar</div>
                            </th>
                            <th class="p-2 whitespace-nowrap">
                    ...
                    <tbody class="text-sm divide-y divide-gray-100">
                      @foreach ($getMaestros as $maestro)
                      <tr>
                          <td class="p-2 whitespace-nowrap">
                              <div class="flex items-center">
                                  <div class="font-medium text-gray-800">{{$maestro->id}}</div>
                              </div>
                          </td>
                          <td class="p-2 whitespace-nowrap">
                              <div class="text-left">{{$maestro->nombre_maestro}}</div>
                          </td>
                          <td class="p-2 whitespace-nowrap">
                              <div class="text-left font-medium text-green-500">{{$maestro->salon}}</div>
                          </td>
                          <td class="p-2 whitespace-nowrap">
                              <div class="text-left font-medium text-green-500">{{$maestro->ciclo_escolar}}</div>
                          </td>
                          <td class="p-2 whitespace-nowrap">
                                      ...
                                      <div class="flex items-end justify-center min-h-screen px-4 text-center md:items-center sm:block sm:p-0">
                                          ...
                                          <div x-cloak x-show="modelOpen" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-xl p-8 my-20 overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl 2xl:max-w-2xl">
                                              <div class="flex items-center justify-between space-x-4">
                                                  <h2 class="text-xl font-medium text-gray-800">Editar la información del Profesor</h2>
                                              ...
                                              <form class="mt-5" action="" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                {{ method_field('PUT') }}
                                              <h1 class="text-xs font-medium text-gray-400 uppercase">Profesor</h1>
                                                  <div>
                                                      <label for="user name" class="block text-sm text-black capitalize dark:text-gray-200">Id</label>
                                                      <input placeholder="Apellido" value="{{$maestro->id}}" disabled type="text" class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                                                  </div>
                                                  <div>
                                                      <label for="user name" class="block text-sm text-black capitalize dark:text-gray-200">Salon de clases</label>
                                                      <input placeholder="Nombre" value="{{$maestro->salon}}" id="inicio_ciclo" name="inicio_ciclo" type="text" class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                                                  </div>
                                                  <div>
                                                      <label for="user name" class="block text-sm text-black capitalize dark:text-gray-200">Ciclo Escolar</label>
                                                      <input placeholder="Apellido" value="{{$maestro->ciclo_escolar}}" id="fin_ciclo" name="fin_vigencia" type="text" class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                                                  </div>
